...
if (typeof Scratch !== "undefined") {
  ...
  fs.writeFileSync("lol.json", JSON.stringify(utils.generateFullAST({
    CODE: `import as "glass" from "packages"
...
s = 0
frame rw / -2 - s rh / 2 + s rw / 2 + s rh / -2 - s
goto 0 0
c window_colour
pen "opacity" 40
icon "w 10 dot 0 0" frame.width + frame.height
c txtc
icongrid rw / w rh / h w h new_grid
frame "clear"
loc 2 -2 20 20
icon playing ? "pause" "play" 1
if onclick (
  playing.not()
)
icon "reload" 0.8 : chx#30
if onclick (
  reset()
)
text fps 10 : chx#30
